...
{
    private protected XUnitV3FiltersCollection _filters = new();
        ...
        if (tests.Any())
        {
            ...
            foreach (var t in tests)
            {
                if (t.StartsWith("KLASS:", StringComparison.Ordinal))
                {
                    ...
                    _filters.Add(XUnitV3Filter.CreateClassFilter(klass, true));
                }
                ...
                else if (t.StartsWith("Platform32:", StringComparison.Ordinal) && IntPtr.Size == 4)
                {
                    ...
                    _filters.Add(XUnitV3Filter.CreateSingleFilter(filter, true));
                }
                else
                {
                    _filters.Add(XUnitV3Filter.CreateSingleFilter(t, true));
                }
            }
        }
    }
    ...
    public virtual void SkipCategories(IEnumerable<string> categories, bool isExcluded)
    {
        ...
        foreach (var c in categories)
        {
            ...
            {
                _filters.Add(XUnitV3Filter.CreateTraitFilter(traitInfo[0], traitInfo[1], isExcluded));
            }
            else
            {
                _filters.Add(XUnitV3Filter.CreateTraitFilter(c, null, isExcluded));
            }
        }
    }

    public override void SkipMethod(string method, bool isExcluded)
        => _filters.Add(XUnitV3Filter.CreateSingleFilter(singleTestName: method, exclude: isExcluded));

    public override void SkipClass(string className, bool isExcluded)
        => _filters.Add(XUnitV3Filter.CreateClassFilter(className: className, exclude: isExcluded));

    public virtual void SkipNamespace(string namespaceName, bool isExcluded)
        => _filters.Add(XUnitV3Filter.CreateNamespaceFilter(namespaceName, exclude: isExcluded));
}